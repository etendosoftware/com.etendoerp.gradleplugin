...
class GithubUtils {
    ...
    static askCredentials(Project project) {
        ...
        if (!githubUser || !githubToken) {
            def input = project.getServices().get(UserInputHandler)
            githubUser = project.getServices().get(UserInputHandler).askQuestion('GitHub user', '')
            githubToken = project.getServices().get(UserInputHandler).askQuestion('GitHub Token', '')
